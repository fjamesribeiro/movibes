...
# (Este arquivo foi gerado, mas o conteúdo abaixo foi preenchido manualmente)
...
CATEGORIAS_INICIAIS = [
    {'nome': 'Bike', 'icone': '🚲'},
    {'nome': 'Basquete', 'icone': '🏀'},
    {'nome': 'CrossFit', 'icone': '🏋️'},
    {'nome': 'Yoga', 'icone': '🧘'},
    {'nome': 'Funcional', 'icone': '👟'},
    {'nome': 'Natação', 'icone': '🏊'},
    {'nome': 'Futvôlei', 'icone': '⚽'},
    {'nome': 'Beach Tênis', 'icone': '🎾'},
    {'nome': 'Futebol', 'icone': '⚽'},
    {'nome': 'Vôlei', 'icone': '🏐'},
    {'nome': 'Corrida', 'icone': '🏃'},
]
...
def popular_categorias(apps, schema_editor):
    ...
    try:
        ...
        print(
            "Por favor, ajuste o 'get_model('nome_do_app', 'Categoria')' no arquivo de migração.")
        raise
    ...
    for cat_data in CATEGORIAS_INICIAIS:
        ...
        if created:
            ...
            print(f"  -> Categoria já existe: {obj.nome}")


def remover_categorias(apps, schema_editor):
    """
    Remove as categorias de evento criadas. (Função reversa)
    """
